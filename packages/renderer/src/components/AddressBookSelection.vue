<template>
  <div>
    <div class="flex justify-between px-5">
      <label class="block text-sm font-medium text-on_background">Addressbook Entries</label>
      <!-- Add Members Button -->
      <NormalButton
  ...
  <FloatingModal
    ...
    title="Select Addressbook Entries"
  ...
  >
    <div class="mb-6 w-96 max-w-sm">
      ...
      <PaginationBar
        ...
        :total-retrievable-entities="totalAddressbookAmount()"
        :page-size="5"
...
<script lang="ts" setup>
  ...
  const addressbooks = computed(() => addressbookState.getters.entries);
  const addressbookPage = computed(() => addressbookState.getters.page);
  const totalAddressbookAmount = computed(() => addressbookState.getters.total);
  const addressbookSearchResults = computed(() => addressbookState.getters.searchResults);
  const addressbookSearchResultsArray = computed(() => {
    return IterableIteratorToArray(addressbookSearchResults.value().values());
  });
  const usersPageArray = computed(() => {
    // change user iterables to arrays for use with searchable select
    if(props.include) {
      return IterableIteratorToArray(addressbookPage.value().values()).filter((elem) => !props.modelValue.includes(elem?.id? elem?.id : '') && props.includeIds?.includes(elem?.id? elem?.id : ''));
    } else {
      return IterableIteratorToArray(addressbookPage.value().values()).filter((elem) => !props.modelValue.includes(elem?.id? elem?.id : ''));
    }
  });
  ...
  const options: Ref<(AddressbookEntry | undefined)[]> = computed(() => {
    if(props.include) {
      return union(addressbookSearchResultsArray.value, arr.value.map((elem) => addressbooks.value().get(elem)))?.filter((elem) => !props.modelValue.includes(elem?.id? elem?.id: '') && props.includeIds?.includes(elem?.id? elem.id : ''));
    } else {
      return union(addressbookSearchResultsArray.value, arr.value.map((elem) => addressbooks.value().get(elem)))?.filter((elem) => !props.modelValue.includes(elem?.id? elem?.id: ''));
    }
  });
  ...
  function handleSelectionInput(query: string) {
    addressbookState.dispatch('searchEntryByQuery', {query, limit:10});
  }
  ...
  function updatePage(amount:number, offset:number) {
    addressbookState.dispatch('retrieveEntries', {amount, offset});
  }
