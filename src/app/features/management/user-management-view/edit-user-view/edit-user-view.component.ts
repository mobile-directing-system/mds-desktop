...
export class EditUserView implements OnInit, OnDestroy {
  ...
  updateUser(): void {
    ...
    if (isActive === undefined) {
      throw new MDSError(MDSErrorCode.AppError, 'is_active control is not set');
    }
    const firstName = this.form.value.firstName;
    if (firstName === undefined) {
      throw new MDSError(MDSErrorCode.AppError, 'first_name control is not set');
    }
    const lastName = this.form.value.lastName;
    if (lastName === undefined) {
      throw new MDSError(MDSErrorCode.AppError, 'last_name control is not set');
    }
